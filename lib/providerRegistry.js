...
const PROVIDER_SPECS = [
  ...
  },
  {
    ...
    patterns: [/\bsk-(?:proj-|svcacct-)?[A-Za-z0-9_-]{20,}/],
  ...
  },
  {
    ...
    highConfidence: true,
    validation: { type: "webhook-json", urlFromToken: true, body: { name: "apikeyvalidator-validation" } }
  },
  {
    id: "zendesk",
    company: "Zendesk",
...
function requestFromValidation(validation, token) {
  ...
  switch (validation.type) {
    ...
    case "mailchimp": {
      ...
      headers.authorization = `Basic ${Buffer.from(`apikeyvalidator:${token}`).toString("base64")}`;
      break;
    }
    case "slack":
